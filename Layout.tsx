...
const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // FIX: Removed redundant frontend logging, as this action is logged by the backend.
            showToast(t('changePassword.success'), 'success');
            onClose();
        } catch (err: any) {
            setError(err.message || t('errors.incorrectPassword'));
            console.error(err);
        } finally {
            setIsSubmitting(false);
        }
          ...
          <div className="flex items-center justify-between h-full">
            ...
            <div className="flex items-center space-x-5 rtl:space-x-reverse">
                              ...
                              {notifications.length > 0 ? notifications.map(notif => (
                                  <li key={notif.id} className={`px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 ${!notif.read ? 'bg-primary-50 dark:bg-primary-900/40' : ''}`}>
                                      <div className="flex items-start">
                                          <i className={`fa-solid ${getNotificationIcon(notif.type)} mt-1`}></i>
              ...
              <div className="relative" ref={userDropdownRef}>
                ...
                {userDropdownOpen && (
                  <div className={`absolute z-50 my-4 w-56 text-base list-none bg-white divide-y divide-slate-100 rounded shadow dark:bg-slate-700 dark:divide-slate-600 ${language === 'ar' ? 'left-0' : 'right-0'}`}>
                    <div className="px-4 py-3">
                       <div className="flex items-center mb-1">
                          <i className="fa-solid fa-user w-4 text-slate-400" aria-hidden="true"></i>
                          <span className="ms-2 block text-sm text-slate-900 dark:text-white truncate">{user?.username}</span>
                       </div>
                       <div className="flex items-center">
                    ...
                    <ul className="py-1" aria-labelledby="dropdown">
                      <li>
                        <a href="#" className="flex items-center px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 dark:text-slate-200 dark:hover:text-white">
                            <i className="fa-solid fa-user-circle w-5 text-slate-400" aria-hidden="true"></i>
                            <span className="ms-2">{t('layout.profile')}</span>
                        </a>
                      </li>
                      <li>
                        <button onClick={() => setIsChangePasswordModalOpen(true)} className="flex items-center w-full text-start px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 dark:text-slate-200 dark:hover:text-white">
                            <i className="fa-solid fa-key w-5 text-slate-400" aria-hidden="true"></i>
                            <span className="ms-2">{t('layout.changePassword')}</span>
                        </button>
                      </li>
                      <li>
                        <button onClick={handleLogout} className="flex items-center w-full text-start px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 dark:text-slate-200 dark:hover:text-white">
                          <i className="fa-solid fa-right-from-bracket w-5 text-slate-400" aria-hidden="true"></i>
                          <span className="ms-2">{t('layout.signOut')}</span>
                        </button>
                      </li>
                    </ul>
                  </div>
          ...
          <ul className="space-y-1.5 pt-2">
            {navLinks.map(link => link.visible && (
              <li key={link.to}>
                ...
                >
                  <i className={`fa-solid ${link.icon} w-5 h-5 transition duration-75 ${'text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400'}`}></i>
