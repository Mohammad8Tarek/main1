...
const ReservationsPage: React.FC = () => {
    ...
    const [pagination, setPagination] = useState({
        assignments: { currentPage: 1, rowsPerPage: 10 },
        reservations: { currentPage: 1, rowsPerPage: 10 },
        hosting: { currentPage: 1, rowsPerPage: 10 },
        history: { currentPage: 1, rowsPerPage: 10 },
    });
    const updatePagination = (tab: keyof typeof pagination, newPage: number, newRows?: number) => {
        setPagination(p => ({
            ...p,
            [tab]: {
                currentPage: newPage,
                rowsPerPage: newRows || p[tab].rowsPerPage
            }
        }));
    };
    ...
    const getExportData = () => {
        ...
        switch (activeTab) {
            case 'assignments':
                reportTitle = t('reservations.assignmentReportTitle');
                ...
                break;
            case 'reservations':
                reportTitle = t('reservations.reservationReportTitle');
            ...
            case 'hosting':
                ...
                break;
            case 'history':
                reportTitle = t('reservations.assignmentHistoryReportTitle');
                headers = [t('employees.firstName'), t('employees.lastName'), t('employees.jobTitle'), t('housing.building'), t('housing.roomNumber'), t('reservations.checkIn'), t('reservations.checkOut')];
                data = assignments.filter(a => a.checkOutDate).sort((a,b) => new Date(b.checkOutDate!).getTime() - new Date(a.checkOutDate!).getTime()).map(item => {
                    const employee = employeeMap.get(item.employeeId);
                    return [
                        employee ? employee.firstName : t('unknown'),
                        employee ? employee.lastName : '',
    ...
    const renderContent = () => {
        ...
        const tabKey = activeTab as keyof typeof pagination;
        const { currentPage, rowsPerPage } = pagination[tabKey];
        
        const renderPaginatedTable = (items: any[], renderRow: (item: any) => React.ReactElement, headers: string[]) => {
            ...
            return (
                <>
                    ...
                    <PaginationControls
                        ...
                        onPageChange={(page) => updatePagination(tabKey, page)}
                        rowsPerPage={rowsPerPage}
                        onRowsPerPageChange={(rows) => updatePagination(tabKey, 1, rows)}
        ...
        switch (activeTab) {
            case 'assignments':
                return renderPaginatedTable(
                    assignments.filter(a => !a.checkOutDate),
                    (a: Assignment) => {
                        ...
                        return (
                            <tr key={a.id} className="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600">
                                ...
                                {canManage && <td className="px-6 py-4"><button onClick={() => openCheckOutModal(a)} className="font-medium text-primary-600 dark:text-primary-500 hover:underline">{t('reservations.checkoutButtonLabel')}</button></td>}
                            </tr>
                        );
                    },
                    [t('employees.firstName'), t('employees.lastName'), t('employees.jobTitle'), t('housing.building'), t('housing.roomNumber'), t('reservations.checkIn'), t('reservations.expectedCheckOut'), ...(canManage ? [t('actions')] : [])]
                );
            case 'reservations':
                 return renderPaginatedTable(
                    reservations,
                    ...
                    [t('reservations.guestName'), t('reservations.jobTitle'), t('housing.roomNumber'), t('employees.department'), t('reservations.checkInDate'), ...(canManage ? [t('actions')] : [])]
                );
             case 'hosting':
                 return renderPaginatedTable(
                    hostings,
            ...
            case 'history':
                return renderPaginatedTable(
                    assignments.filter(a => a.checkOutDate).sort((a,b) => new Date(b.checkOutDate!).getTime() - new Date(a.checkOutDate!).getTime()),
                     (a: Assignment) => {
                        const employee = employeeMap.get(a.employeeId);
                        return (
                            <tr key={a.id} className="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600">
                                ...
                                <td className="px-6 py-4">{roomMap.get(a.roomId) || t('unknown')}</td>
                                <td className="px-6 py-4">{new Date(a.checkInDate).toLocaleString()}</td>
                                <td className="px-6 py-4">{a.checkOutDate ? new Date(a.checkOutDate).toLocaleString() : '—'}</td>
                            </tr>
                        );
                    },
                    [t('employees.firstName'), t('employees.lastName'), t('housing.building'), t('housing.roomNumber'), t('reservations.checkIn'), t('reservations.checkOut')]
                );
        }
    };
                ...
                <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md">
                    <div className="p-4 flex flex-wrap justify-between items-center border-b dark:border-slate-700 gap-4">
                        <div className="flex items-center space-x-2 rtl:space-x-reverse flex-wrap gap-y-2">
                            ...
                            <button onClick={() => setActiveTab('history')} className={`px-3 py-2 text-sm rounded-md ${activeTab === 'history' ? 'bg-primary-600 text-white font-semibold' : 'bg-slate-200 dark:bg-slate-700'}`}>{t('reservations.assignmentHistory')}</button>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {canManage && activeTab === 'assignments' && <button onClick={openNewAssignmentModal} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-sm"><i className="fa-solid fa-plus me-2"></i>{t('reservations.new')}</button>}
                            {canManage && activeTab === 'reservations' && <button onClick={openNewReservationModal} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-sm"><i className="fa-solid fa-plus me-2"></i>{t('reservations.newRes')}</button>}
                             {canManage && activeTab === 'hosting' && <button onClick={openNewHostingModal} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-sm"><i className="fa-solid fa-plus me-2"></i>{t('reservations.newHosting')}</button>}
                        </div>
                    </div>
                     {renderContent()}
                </div>
            ...
            {isCheckOutModalOpen && assignmentToCheckOut && (
                 <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-md">
                        ...
                        <div className="mb-6"><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">{t('reservations.checkOutDate')}</label><input type="datetime-local" value={checkOutDate} onChange={e => setCheckOutDate(e.target.value)} required className={formInputClass}/></div>
