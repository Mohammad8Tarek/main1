...
const EmployeesPage: React.FC = () => {
    ...
    const processImportedData = (data: any[]) => {
        ...
        data.forEach((rawRow, index) => {
            ...
            if (!firstName) {
                error = t('employees.import.error.missingValue', { field: t('employees.firstName') });
            } else if (!row.employeeId) {
                error = t('employees.import.error.missingValue', { field: t('employees.employeeId') });
            } else if (!row.nationalId) {
                error = t('employees.import.error.missingValue', { field: t('employees.nationalId') });
            } else {
                const nationalIdStr = String(row.nationalId).trim();
                if (existingNationalIds.has(nationalIdStr)) {
                    error = t('employees.import.error.duplicateNationalIdInDb', { nationalId: nationalIdStr });
                } else if (fileNationalIds.has(nationalIdStr)) {
                    error = t('employees.import.error.duplicateNationalIdInFile', { nationalId: nationalIdStr });
                }

                if (!error) {
                    const employeeIdStr = String(row.employeeId).trim();
                    if (existingEmployeeIds.has(employeeIdStr)) {
                        error = t('employees.import.error.duplicateEmployeeIdInDb', { employeeId: employeeIdStr });
                    } else if (fileEmployeeIds.has(employeeIdStr)) {
                        error = t('employees.import.error.duplicateEmployeeIdInFile', { employeeId: employeeIdStr });
                    }
                }
            }
            
            const departmentKey = !error ? departmentValueToKeyMap[String(row.department || '').trim().toLowerCase()] : null;
            if (!error && !departmentKey) {
                error = t('employees.import.error.invalidDepartment', { value: String(row.department) });
            }
            ...
            if (!error && !(row.contractEndDate instanceof Date && !isNaN(row.contractEndDate.getTime()))) {
                error = t('employees.import.error.invalidDate');
            }
    ...
    const pageEmployeeIds = useMemo(() => paginatedEmployees.map(emp => emp.id), [paginatedEmployees]);
    const selectedOnPageCount = useMemo(() => pageEmployeeIds.filter(id => selectedEmployees.has(id)).length, [pageEmployeeIds, selectedEmployees]);

    const isAllOnPageSelected = paginatedEmployees.length > 0 && selectedOnPageCount === paginatedEmployees.length;
    const isPageIndeterminate = selectedOnPageCount > 0 && selectedOnPageCount < paginatedEmployees.length;
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md">
                    <div className="p-4 border-b dark:border-slate-700 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                         ...
                         <div className="flex justify-end items-center gap-2">
                            ...
                            {canManage && <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm" disabled={isSubmitting}><i className="fa-solid fa-file-import me-2"></i>{t('importExcel')}</button>}
                            <input type="file" ref={fileInputRef} onChange={handleFileChange} style={{ display: 'none' }} accept=".xlsx, .xls" />
                            {canManage && <button onClick={openAddModal} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 text-sm"><i className="fa-solid fa-plus me-2"></i>{t('employees.add')}</button>}
                        </div>
                    </div>
                        ...
                        <div className="relative overflow-x-auto">
                            <table className="w-full text-sm text-left rtl:text-right text-slate-500 dark:text-slate-400">
                                <thead className="text-xs text-slate-700 uppercase bg-slate-100 dark:bg-slate-700 dark:text-slate-400">
                                    <tr>
                                        {canManage && <th scope="col" className="p-4"><input type="checkbox" ref={el => { if (el) { el.indeterminate = isPageIndeterminate; } }} checked={isAllOnPageSelected} onChange={handleSelectAllOnPage} className="w-4 h-4 text-primary-600 bg-gray-100 border-gray-300 rounded focus:ring-primary-500 dark:focus:ring-primary-600" aria-label={t('employees.selectAll')} /></th>}
                    ...
                    <PaginationControls
                        ...
                        filteredRowCount={filteredEmployees.length}
                    />
                </div>
            </div>
